...
<body>
    ...
    <?php
        ...
        $selected_visibility = isset($_POST['visibility']) ? $_POST['visibility'] : null;
        $visibilitypost = null;

        // Gán giá trị visibilitypost dựa trên selected_visibility
        if ($selected_visibility == 1) {
            $visibilitypost = "public"; // Công khai
        } elseif ($selected_visibility == 2) {
            $visibilitypost = "protected"; // Thành viên
        } else {
            $visibilitypost = "private"; // Riêng tư
        }
        ...
        if (isset($_POST['btn-add'])) {
            if($idpost == 0){
                echo $PostBLL->insertPost($_POST['title'], $_SESSION['imagepost'], $_POST['content'], $ideapost, $iduser, $_POST['description'], $visibilitypost);
            }else{
                echo $PostBLL->updatePost($idpost, $_POST['title'], $_SESSION['imagepost'], $_POST['content'], $ideapost, $_POST['description'], $visibilitypost);
            }
        }
    ?>
    ...
    <div id="wrapper">
        ...
        <form action="" method="post">
            ...
            <br><div class="radio-group">
                <label><input type="radio" name="visibility" value="1" checked>Công khai</label>
                <label><input type="radio" name="visibility" value="2">Thành viên</label>
                <label><input type="radio" name="visibility" value="3">Riêng tư</label>
            </div><br>

            <textarea style="width: 100%;" rows="5" name="description" placeholder="Mô tả nhanh"></textarea>
