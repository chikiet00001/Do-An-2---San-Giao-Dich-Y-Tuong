...
class UserBLL{
    ...
    public function updateUser($iduser, $username, $passwd, $email, $fullname, $dod, $organize, $phonenumber, $address, $status, $role) {
        // Kiểm tra các giá trị đầu vào (ví dụ: không để trống)
        if (empty($username) || empty($passwd) || empty($email) || empty($fullname)) {
            return "Các trường quan trọng không được để trống.";
        }
        ...
        return $this->userDAL->updateUser($iduser, $username, $passwd, $email, $fullname, $dod, $organize, $phonenumber, $address, $status, $role);
    }
    ...
    public function getUserDetails($userId) {
        ...
        if ($user) {
            // Ví dụ: kiểm tra trạng thái tài khoản
