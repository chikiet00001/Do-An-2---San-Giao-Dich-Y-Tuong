...
class UserDAL{
    ...
    public function AllUsers() {
        $sql = "SELECT iduser, username, passwd, email, fullname, dod, organize, 
                phonenumber, address, createdat, updatedat, status, role 
                FROM users";
    ...
    public function updateUser($iduser, $username, $passwd, $email, $fullname, $dod, $organize, $phonenumber, $address, $status, $role) {
        $sql = "UPDATE users 
                SET username = ?, passwd = ?, email = ?, fullname = ?, dod = ?, organize = ?, phonenumber = ?, address = ?, status = ?, role = ?
                WHERE iduser = ?";
        $stmt = $this->db->conn->prepare($sql);
        $stmt->bind_param(
            "ssssssssssi", 
            $username, $passwd, $email, $fullname, $dod, $organize, $phonenumber, $address, $status, $role, $iduser
        );
    ...
    public function getUserById($userId) {
        $query = "SELECT iduser, username, passwd, email, fullname, dod, organize, 
                         phonenumber, address, createdat, updatedat, status, role
                  FROM users WHERE iduser = ?";
        $stmt = $this->db->conn->prepare($query);
        $stmt->bind_param("i", $userId); // "i" đại diện cho kiểu dữ liệu integer
        $stmt->execute();
